...
async function testPRCreationPagePrompt(config, prDetails) {
  ...
  console.log(`Existing body length: ${existingBody.length} chars (simulated empty)`);
  ...
  console.log(`Prompt includes Commit Coverage section: ${hasCommitCoverageSection ? 'Yes' : 'No'} (expected: Yes - no existing body)`);
  
  // Test passes if: prompt includes all commits AND prompt has commit coverage instruction
  const promptHasAllCommits = coveredInSummary === commits.length;
  const promptHasCoverageInstruction = hasCommitCoverageSection;
  
  if (promptHasAllCommits && promptHasCoverageInstruction) {
    console.log(`\n✅ TEST PASSED: PR creation page prompt includes all commits and coverage instruction`);
    return { passed: true };
  } else if (promptHasAllCommits) {
...
async function main() {
  console.log('=== GitHub PR Generator - PR Creation Page Prompt Test ===');
  ...
  try {
    ...
    const result = await testPRCreationPagePrompt(config, prDetails);
